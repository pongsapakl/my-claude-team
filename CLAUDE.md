...
- **Plugin membership rules are explicit.** `handover` = skills that read/write the project file contract; `session` = skills that operate on the conversation; `utils` = maintainer tools for this repo. A new plugin must come with a membership rule — a group without one becomes a junk drawer, and installing it means taking unrelated skills at once, which is what separate plugins exist to prevent.
